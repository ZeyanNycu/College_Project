...
class ESFMLoss(nn.Module):
    ...
    def forward(self, pred_cam, data, epoch=None):
        ...
        if self.normalize_grad:
            pts_2d.register_hook(lambda grad: F.normalize(grad, dim=1) / (data.valid_pts.sum() + eplison))
        ...
        pts_2d = (pts_2d / (torch.where(projected_points, pts_2d[:, 2, :], torch.ones_like(projected_points).float()).unsqueeze(dim=1)+eplison))
