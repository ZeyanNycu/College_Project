    ...
    with torch.no_grad():
        for batch_data in data_loader:
            for curr_data in batch_data:
                ...
                image_path = f"/media/user/data1/zeyan/Equivariant-SFM/datasets/kaggle/train/{curr_data.scan_name}/images"
                image_names = [ i for i in os.listdir(image_path) ]
                image_names.sort()
                
                if save_predictions:
                    dataset_utils.save_cameras(outputs, conf, curr_epoch=epoch, phase=phase)
                    if conf.get_bool('dataset.calibrated'):
                        path = plot_utils.plot_cameras_before_and_after_ba(outputs, errors, conf, phase, curr_data.xs, image_path, image_names, scan=curr_data.scan_name, epoch=epoch, bundle_adjustment=bundle_adjustment)
