...
def plot_cameras_before_and_after_ba(outputs, errors, conf, phase, x, image_path, image_names, scan, epoch=None, bundle_adjustment=False):
    Rs_gt = outputs['Rs_gt']
    ts_gt = outputs['ts_gt']
    ...
    plot_cameras(Rs_pred, ts_pred, pts3D, Rs_gt, ts_gt, Rs_error, ts_error, conf, phase, x, image_path, image_names, scan=scan, epoch=epoch)
    ...
    if bundle_adjustment:
        ...
        plot_cameras(Rs_pred, ts_pred, pts3D, Rs_gt, ts_gt, Rs_error, ts_error, conf, phase, x, image_path, image_names, scan=scan+'_ba', epoch=epoch)
...
def get_points_colors(images_path, image_names, xs, first_occurence=False):
    ...
    if first_occurence:
        ...
        for image_ind in range(m):
            image_name = str(image_names[image_ind])
            im = image.imread(os.path.join(images_path, image_name))
...
def plot_cameras(Rs_pred, ts_pred, pts3D, Rs_gt, ts_gt, Rs_error, ts_error, conf, phase, x, image_path, image_names, scan=None, epoch=None):
    data = []
    ...
    data.append(get_3D_scater_trace(pts3D, get_points_colors(image_path, image_names, x), '3D points', size=0.5))
