...
void MONTE_CARLO_SYSTEM::computeSamples( )
{
	...
	{
		x=(mMaxX-mMinX)*((double)rand()/(double)(RAND_MAX))+mMinX;
		y=(mMaxY-mMinY)*((double)rand()/(double)(RAND_MAX))+mMinY;
		mX.push_back(x);
		mY.push_back(y);
	}
...
double MONTE_CARLO_SYSTEM::computeArea( ) const
{
	...
	area=(mMaxX-mMinX)*(mMaxY-mMinY)*count/(double)mNumSamples;
    cout << "Area:" << area << endl;
    return area;
